...
- 保存设计树时会同步生成 Hibist / Sailor / Verification 的模块级配置骨架。
...
|       |-- flows/                  # Common、Design (Hibist/Sailor)、Verification 容器
...
`App.tsx` 负责主题适配、顶层路由，以及渲染 `Welcome`、`CommonFlow`、`DesignFlow` (渲染 Hibist/Sailor)、`VerificationFlow` 或旧 wizard fallback。Formal 和 STA 当前是规划入口，后续可按 Hibist / Sailor / Verification 模式补齐。
...
Hibist/Sailor 和 Verification 使用 `src/webview/components/shared/DesignTreePanel.tsx` 选择和维护模块范围。
...
保存设计树时还会更新 Hibist / Sailor / Verification 的模块配置骨架，包括模块 key、标题、类型、当前模块，以及每个模块自己的 config 文件。
